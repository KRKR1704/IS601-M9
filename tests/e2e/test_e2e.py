...
def test_calculator_add(page, fastapi_server):
    ...
    # Wait for the result div to be updated by the page's async fetch handler, then assert
    page.wait_for_function("() => document.querySelector('#result') && document.querySelector('#result').innerText.trim() !== ''")
...
def test_calculator_divide_by_zero(page, fastapi_server):
    ...
    # Wait for the result div to be updated, then assert the error message is shown
    page.wait_for_function("() => document.querySelector('#result') && document.querySelector('#result').innerText.trim() !== ''")
...
def test_calculator_power(page, fastapi_server):
    """
    Test the power functionality on the frontend.
    """
    page.goto('http://localhost:8000')
    page.fill('#a', '2')
    page.fill('#b', '3')
    ...
    page.wait_for_function("() => document.querySelector('#result') && document.querySelector('#result').innerText.trim() !== ''")
    assert page.inner_text('#result').strip() == 'Result: 8'
